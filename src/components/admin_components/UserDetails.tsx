...
interface Subscription {
	...
	start_date: string;
	expiry_date: string;
	status: string; // e.g. "cancelled" or others
}
...
const UserDetails = () => {
	...
	// Define sorting order for subscription statuses
			...
			{user.subscriptions && user.subscriptions.length > 0 && (
				<div className="mt-6 max-w-5xl">
					...
					<ul className="space-y-4">
						{[...user.subscriptions]
							.sort((a, b) => {
								const statusA = getSubscriptionStatus(
									a.start_date,
									a.expiry_date,
									a.status === "cancelled"
								) as SubscriptionStatus;
								const statusB = getSubscriptionStatus(
									b.start_date,
									b.expiry_date,
									b.status === "cancelled"
								) as SubscriptionStatus;
