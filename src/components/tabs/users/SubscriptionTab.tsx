...
const SubscriptionTab = () => {
	...
	const sortedPayments = [...payments].sort((a, b) => {
		...
		const getStatus = (p: Payment): string => {
			...
			const isCancelled = p.status === "cancelled" || Boolean(p.deleted_at);

			if (isCancelled && now >= start && now <= end)
				return "cancelled (active)";
			if (!isCancelled && now >= start && now <= end) return "currently active";
	...
	const current =
		sortedPayments.find((p) => {
			const start = new Date(p.date);
			const end = new Date(p.expiry_date);
			return (
				(!p.deleted_at && p.status !== "cancelled" && now <= end) ||
